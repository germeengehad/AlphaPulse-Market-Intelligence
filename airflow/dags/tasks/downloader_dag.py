...
import pandas as pd
from constants.ingestion_constants import TICKERS, INTERVALS, START_DATE, SLEEP_BETWEEN, LOOKBACK
...
def download_one(symbol: str, interval: str, out_dir: Path):
    ...
    kwargs = {}
    if interval == "1m":
        kwargs = {"period": "60d", "interval": "1m", "progress": False, "threads": False}
    else:
        period_days = LOOKBACK.get(interval)
        if period_days == "max":
            kwargs = {"start": START_DATE, "interval": interval, "progress": False, "threads": False}
        else:
            if "y" in period_days:
                days = int(period_days.replace("y", "")) * 365
            else:
                days = int(period_days.replace("d", ""))
            start_ts = (pd.Timestamp.utcnow() - pd.Timedelta(days=days)).strftime("%Y-%m-%d")
            kwargs = {"start": start_ts, "interval": interval, "progress": False, "threads": False}
